    ...
    while T.value() <= args.T_max:
        ...
        while True:
            ...
            while not done and t - t_start < args.t_max:
                ...
                if gpu_id >= 0:
                    ...
                    to_avg_state = state
                average_policy, _, _, (avg_hx, avg_cx) = shared_average_model(to_avg_state, (avg_hx, avg_cx))
                # if gpu_id >= 0:
        ...
        if not args.on_policy and len(memory) >= args.replay_start:
            ...
            for _ in range(_poisson(args.replay_ratio)):
                ...
                if gpu_id >= 0:
                    with torch.cuda.device(gpu_id):
                        hx = torch.zeros(args.batch_size, args.hidden_size).cuda()
                        cx = torch.zeros(args.batch_size, args.hidden_size).cuda()
                else:
